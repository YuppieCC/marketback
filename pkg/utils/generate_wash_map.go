package utils  // 修改这里，从 package scripts 改为 package main
...
// GreedySplit 将一个数字 n 分割成多个不小于 minVal 且不大于 maxVal 的数字，maxLen 限制输出数组的最大长度
func GreedySplit(n int, minVal int, maxVal int, maxLen int) []int {
	var result []int
	remain := n
	...
	for remain >= minVal {
		...
		// 取最大可能值，不超过 maxVal
		val := min(remain, maxVal)

		// 如果 val < minVal，说明不能再加了
		if val < minVal {
			break
		}
